...
export type GaleryPopupProps = {
    open?: boolean,
    onClose?: (state: boolean, images: string[]) => void,
    images?: string[],
}

export function GaleryPopup(props: GaleryPopupProps) {
    const [images, setImages] = React.useState(props.images ? props.images :
        [
    ...
    return (
        <Popup open={props.open} onClose={(state) => props.onClose(state, images)} className='GaleryPopup'>
            <header>
                <a className='title'>
